...
draw.023 <- function(
	...
	for (i in 1:3) {
		...
		if (0 == tmp$areas[1] & 0 == tmp$areas[3] & 0 == tmp$areas[4] & 0 == tmp$areas[6] & 0 == tmp$areas[7]) { break; }
		}
	...
	return(
		VennDiagram::draw.sp.case(
			area.list = c(0, a2, 0, 0, a5, 0, 0),
			enabled.areas = c(2, 5),
			area.x = c(0, a2.x.pos, 0, 0, a5.x.pos, 0, 0),
			area.y = c(0, a2.y.pos, 0, 0, a5.y.pos, 0, 0),
			attach.label.to = c(2, 2, 5),
